...
function s.initial_effect(c)
    c:EnableReviveLimit()
    s.AddProcedure(c,aux.FilterBoolFunctionEx(Card.IsSetCard,0x29e),3,2)
    ...
    e4:SetTarget(aux.TargetBoolFunction(Card.IsSetCard,0x29e))
    c:RegisterEffect(e4)
    --Add Oberon
...
function s.faefil(c,tp,xyzc)
    return c:IsSetCard(0x29e,xyzc,SUMMON_TYPE_XYZ,tp) and (c:IsFaceup() or c:IsFacedown())
end
...
--Start of custom procedure (literally copy pasted it lol)
...
s.ProcCancellable=false
function s.EffectXyzMaterialChk(c,xyz,tp)
...
function s.AlterFilter(c,alterf,xyzc,e,tp,op)
    if not alterf(c,tp,xyzc) or not c:IsCanBeXyzMaterial(xyzc,tp)
        or (c:IsControler(1-tp) and not s.EffectXyzMaterialChk(c,xyzc,tp)) 
...
--Xyz monster, lv k*n
function s.AddProcedure(c,f,lv,ct,alterf,desc,maxct,op,mustbemat,exchk)
...
--mustbemat for Startime Magician
    ...
    chk1:SetCode(946)
    chk1:SetCondition(s.Condition(f,lv,ct,maxct,mustbemat,exchk))
    chk1:SetTarget(s.Target(f,lv,ct,maxct,mustbemat,exchk))
    chk1:SetOperation(s.Operation(f,lv,ct,maxct,mustbemat,exchk))
    ...
    e1:SetCondition(s.Condition(f,lv,ct,maxct,mustbemat,exchk))
    e1:SetTarget(s.Target(f,lv,ct,maxct,mustbemat,exchk))
    e1:SetOperation(s.Operation(f,lv,ct,maxct,mustbemat,exchk))
    ...
    if alterf then
        ...
        chk2:SetCondition(s.Condition2(alterf,op))
        chk2:SetTarget(s.Target2(alterf,op))
        chk2:SetOperation(s.Operation2(alterf,op))
        c:RegisterEffect(chk2)
        local e2=e1:Clone()
        e2:SetDescription(desc)
        e2:SetCondition(s.Condition2(alterf,op))
        e2:SetTarget(s.Target2(alterf,op))
        e2:SetOperation(s.Operation2(alterf,op))
        c:RegisterEffect(e2)
    end
    if not xyztemp then
        xyztemp=true
    ...
    e3:SetOperation(s.MatGenerate)
    Duel.RegisterEffect(e3,0)
    end
end
function s.MatGenerate(e,tp,eg,ep,ev,re,r,rp)
...
--Xyz Summon(normal)
function s.MatFilter2(c,f,lv,xyz,tp)
    if c:IsLocation(LOCATION_GRAVE) and not c:IsHasEffect(511002793) then return false end
    if c:IsLocation(LOCATION_MZONE) and c:IsFacedown() and not c:IsSetCard(0x29e) then return false end
    return s.MatFilter(c,f,lv,xyz,tp)
end
function s.MatFilter(c,f,lv,xyz,tp)
    ...
    and (c:IsControler(tp) or s.EffectXyzMaterialChk(c,xyz,tp))
end
function s.SubMatFilter(c,fil,lv,xg,xyz,tp)
    ...
    return xg:IsExists(s.SubFilterChk,1,nil,fil,xyz,tp)
    end
function s.SubFilterChk(c,f,xyz,tp)
    return (not f or f(c,xyz,SUMMON_TYPE_XYZ|MATERIAL_XYZ,tp))
end
function s.CheckValidMultiXyzMaterial(c,xyz)
...
function s.RecursionChk1(c,mg,xyz,tp,min,max,minc,maxc,sg,matg,ct,matct,mustbemat,exchk,f,mustg,lv)
    local xct=ct
    ...
    local retchknum={0}
    local retchk={mg:IsExists(s.RecursionChk1,1,sg,mg,xyz,tp,min,max,minc,maxc,sg,matg,xct,xmatct,mustbemat,exchk,f,mustg,lv)}
        ...
        for i,te in ipairs(eff) do
            ...
            if not redun and val>0 and (not tgf or tgf(te,xyz)) then
                ...
                if xmatct+val<=maxc then
                    ...
                    table.insert(retchk,mg:IsExists(s.RecursionChk1,1,sg,mg,xyz,tp,min,max,minc,maxc,sg,matg,xct,xmatct+val,mustbemat,exchk,f,mustg,lv))
                end
            end
        end
    end
...
function s.RecursionChk2(c,mg,xyz,tp,minc,maxc,sg,matg,ct,mustbemat,exchk,f,mustg,lv)
    local rg=Group.CreateGroup()
    ...
    local retchknum={0}
    local retchk={mg:IsExists(s.RecursionChk2,1,sg,mg,xyz,tp,minc,maxc,sg,matg,xct,mustbemat,exchk,f,mustg,lv)}
    if not res and c:IsHasEffect(511001225) and not mustbemat then
        local eff={c:GetCardEffect(511001225)}
        for i,te in ipairs(eff) do
            local tgf=te:GetOperation()
            ...
            if val>0 and (not tgf or tgf(te,xyz)) and not redun then
                ...
                if xct+val<=maxc then
                    ...
                    retchk[#retchk+1]=mg:IsExists(s.RecursionChk2,1,sg,mg,xyz,tp,minc,maxc,sg,matg,xct+val,mustbemat,exchk,f,mustg,lv)
                end
            end
        end
    end
...
function s.MatNumChkF(tg)
    ...
    for chkc in aux.Next(chkg) do
        for _,te in ipairs({chkc:GetCardEffect(91110378)}) do
            ...
            if not s.MatNumChk(tg:FilterCount(Card.IsMonster,nil),rct,comp) then return false end
        end
    end
    return true
end
function s.MatNumChk(matct,ct,comp)
    local ok=false
...
function s.MatNumChkF2(tg,lv,xyz)
    ...
    for chkc in aux.Next(chkg) do
        ...
        for _,te in ipairs({chkc:GetCardEffect(86466163)}) do
            local rct=te:GetValue()&0xffff
            local comp=te:GetValue()>>16
            if not s.MatNumChk(tg:FilterCount(Card.IsMonster,nil),rct,comp) then
...
function s.Condition(f,lv,minc,maxc,mustbemat,exchk)
    ...
    return  function(e,c,must,og,min,max)
        ...
        if og then
            g=og
            mg=og:Filter(s.MatFilter,nil,f,lv,c,tp)
        else
            g=Duel.GetMatchingGroup(function(cc) return ((cc:IsLocation(LOCATION_GRAVE) and cc:IsHasEffect(511002793)) or cc:IsFaceup())
            and (cc:IsControler(tp) or s.EffectXyzMaterialChk(cc,c,tp)) end,tp,LOCATION_MZONE+LOCATION_GRAVE,LOCATION_MZONE,nil)
            mg=Duel.GetMatchingGroup(s.MatFilter2,tp,LOCATION_MZONE+LOCATION_GRAVE,LOCATION_MZONE,nil,f,lv,c,tp)
            if not mustbemat then
                ...
                mg:Merge(eqmg)
                mg:Merge(Duel.GetMatchingGroup(s.SubMatFilter,tp,LOCATION_ONFIELD+LOCATION_GRAVE,0,nil,f,lv,xg,c,tp))
            end
        end
        ...
        if min and min~=99 then
            return mg:IsExists(s.RecursionChk1,1,nil,mg,c,tp,min,max,minc,maxc,Group.CreateGroup(),Group.CreateGroup(),0,0,mustbemat,exchk,f,mustg,lv)
        else
            return mg:IsExists(s.RecursionChk2,1,nil,mg,c,tp,minc,maxc,Group.CreateGroup(),Group.CreateGroup(),0,mustbemat,exchk,f,mustg,lv)
        end
        return false
    end
end
function s.Target(f,lv,minc,maxc,mustbemat,exchk)
    return function(e,tp,eg,ep,ev,re,r,rp,chk,c,must,og,min,max)
        ...
        if (#og>=minc and #og<=maxc) or not og:IsExists(Card.IsHasEffect,1,nil,511002116) then
            ...
            while ct<max and matct<maxc do
                local selg=mg:Filter(s.RecursionChk1,sg,mg,c,tp,min,max,minc,maxc,sg,matg,ct,matct,mustbemat,exchk,f,mustg,lv)
                ...
                if not sg:IsContains(sc) then
                    ...
                    elseif sc:IsHasEffect(511001225) then
                        matg:AddCard(sc)
                        ct=ct+1
                        if not s.CheckValidMultiXyzMaterial(sc,c) or (min>=ct and minc>=matct+1) then
                            matct=matct+1
                        else
                            local multi={}
                            if mg:IsExists(s.RecursionChk1,1,sg,mg,c,tp,min,max,minc,maxc,sg,matg,ct,matct+1,mustbemat,exchk,f,mustg,lv) then
                            ...
                            for i=1,#eff do
                                ...
                                if val>0 and (not tgf or tgf(te,c)) then
                                    if (min>=ct and minc>=matct+1+val) 
                                        or mg:IsExists(s.RecursionChk1,1,sg,mg,c,tp,min,max,minc,maxc,sg,matg,ct,matct+1+val,mustbemat,exchk,f,mustg,lv) then
...
--end of part 1
            ...
            if not og or max==99 then
                ...
                local tab={}
                while ct<maxc do
                    local tg=mg:Filter(s.RecursionChk2,sg,mg,c,tp,minc,maxc,sg,matg,ct,mustbemat,exchk,f,mustg,lv)
                    if #tg==0 then break end
                    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_XMATERIAL)
                    local sc=Group.SelectUnselect(tg,sg,tp,finish,cancel)
                    if not sc then
                        if ct<minc or (matg:IsExists(Card.IsHasEffect,1,nil,91110378) and not s.MatNumChkF(matg)) 
                            or (lv and matg:IsExists(Card.IsHasEffect,1,nil,86466163) and not s.MatNumChkF2(matg,lv,c)) then return false end
                    ...
                    if not sg:IsContains(sc) then
                        ...
                        ct=ct+1
                        if s.CheckValidMultiXyzMaterial(sc,c) and ct<minc then
                            local multi={}
                            if mg:IsExists(s.RecursionChk2,1,sg,mg,c,tp,minc,maxc,sg,matg,ct,mustbemat,exchk,f,mustg,lv) then
                                table.insert(multi,1)
                            end
                            local eff={sc:GetCardEffect(511001225)}
                            for i=1,#eff do
                                ...
                                if val>0 and (not tgf or tgf(te,c)) then
                                    ...
                                    or mg:IsExists(s.RecursionChk2,1,sg,mg,c,tp,minc,maxc,sg,matg,ct+val,mustbemat,exchk,f,mustg,lv) then
                                        table.insert(multi,1+val)
                                    end
                                end
                            end
                    ...
                    if ct>=minc and (not matg:IsExists(Card.IsHasEffect,1,nil,91110378) or s.MatNumChkF(matg)) 
                    and (not lv or not matg:IsExists(Card.IsHasEffect,1,nil,86466163) or s.MatNumChkF2(matg,lv,c)) and matg:Includes(mustg) then
                        finish=true
                    end
...
function s.Operation(f,lv,minc,maxc,mustbemat,exchk)
...
--Xyz summon(alterf)
function s.Condition2(alterf,op)
return  function(e,c,must,og,min,max)
if c==nil then return true end
if c:IsType(TYPE_PENDULUM) and c:IsFaceup() then return false end
local tp=c:GetControler()
...
if mustc then
return s.AlterFilter(mustc,alterf,c,e,tp,op)
else
return mg:IsExists(s.AlterFilter,1,nil,alterf,c,e,tp,op)
end
end
end
function s.Target2(alterf,op)
return  function(e,tp,eg,ep,ev,re,r,rp,chk,c,must,og,min,max)
local cancel=not og and Duel.IsSummonCancelable()
s.ProcCancellable=cancel
if og and not min then
...
Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_XMATERIAL)
oc=mg:Filter(s.AlterFilter,nil,alterf,c,e,tp,op):SelectUnselect(Group.CreateGroup(),tp,false,cancel)
end
...
function s.Operation2(alterf,op)
return  function(e,tp,eg,ep,ev,re,r,rp,c,must,og,min,max)
