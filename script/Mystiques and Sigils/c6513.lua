...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
    --Draw equal to number of faceup "Sigil"
...
function s.spbfil(c)
    return c:IsAbleToRemoveAsCost() and ((c:IsSetCard(0x28c)
     and c:IsLocation(LOCATION_SZONE+LOCATION_GRAVE)) or c:IsCode(6502) and c:IsLocation(LOCATION_MZONE))
     and c:IsFaceup()
end
...
function s.drwt(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.IsPlayerCanDraw(tp)
    and Duel.GetMatchingGroupCount(aux.FaceupFilter(Card.IsSetCard,0x28c),tp,LOCATION_SZONE,0,nil)>0 end
    Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,1,0,0)
end
function s.drwop(e,tp,eg,ep,ev,re,r,rp)
    local d=Duel.GetMatchingGroupCount(aux.FaceupFilter(Card.IsSetCard,0x28c),tp,LOCATION_SZONE,0,nil)
    Duel.Draw(tp,d,REASON_EFFECT)
end
function s.ngtfil(c)
    return c:IsAbleToDeck() and c:IsSetCard(0x28c)
    and not c:IsMonster()
end
