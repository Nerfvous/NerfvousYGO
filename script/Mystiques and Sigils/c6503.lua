...
function s.xdcost(e,tp,eg,ep,ev,re,r,rp,chk)
    ...
    local ct=1
    if Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCanBeEffectTarget),tp,0,LOCATION_MZONE,1,nil) then
        ct=2 end
...
function s.xdt(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.IsExistingMatchingCard(s.xdfil,tp,LOCATION_DECK,0,1,nil) end
...
function s.xdop(e,tp,eg,ep,ev,re,r,rp)
    ...
    --Target and destroy
    if lb>=2
    and Duel.IsExistingTarget(aux.FaceupFilter(Card.IsCanBeEffectTarget),tp,0,LOCATION_MZONE,1,nil) then
        Duel.BreakEffect()
        Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
        local sc=Duel.SelectTarget(tp,aux.FaceupFilter(Card.IsCanBeEffectTarget),tp,0,LOCATION_MZONE,1,1,nil)
